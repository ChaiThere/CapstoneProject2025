...
def plot_correlation_graph(merged_crypto_df):
    """
    Plots the rolling correlation between Bitcoin and altcoins for different intervals.
    Also generates a heatmap showing overall correlation.
    ...
    # Define different rolling windows
    intervals = {
        "1 Month (30 days)": 30,
        "3 Months (90 days)": 90,
    ...
    for label, window in intervals.items():
        for crypto in crypto_list:
            if crypto != 'BTC':  # Compare altcoins with BTC
                rolling_corr = merged_crypto_df['BTC_Close'].rolling(window).corr(merged_crypto_df[f'{crypto}_Close'])
                
                # Plot each interval in a separate color
                plt.plot(rolling_corr, label=f'BTC vs {crypto} ({label})')
                
                # Highlight significant inverse correlations (< -0.3)
                significant_points = rolling_corr[rolling_corr < -0.3]
                plt.scatter(significant_points.index, significant_points.values, color='red', marker='o', zorder=3)

    plt.axhline(0, color='black', linestyle='--', alpha=0.6)
    plt.title('Rolling Correlation Between Bitcoin and Altcoins at Different Intervals')
    plt.xlabel('Date')
...
plot_correlation_graph(df_crypto)
